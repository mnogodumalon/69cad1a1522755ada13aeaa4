...
interface WerkzeugkatalogDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Werkzeugkatalog['fields']) => Promise<void>;
  defaultValues?: Werkzeugkatalog['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function WerkzeugkatalogDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: WerkzeugkatalogDialogProps) {
  const [fields, setFields] = useState<Partial<Werkzeugkatalog['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'werkzeugkatalog');
      await onSubmit(clean as Werkzeugkatalog['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Bezeichnung\n  "inventarnummer": string | null, // Inventarnummer\n  "kategorie": LookupValue | null, // Kategorie (select one key: "handwerkzeug" | "elektrowerkzeug" | "messgeraet" | "pruefgeraet" | "maschine" | "sonstiges") mapping: handwerkzeug=Handwerkzeug, elektrowerkzeug=Elektrowerkzeug, messgeraet=Messgerät, pruefgeraet=Prüfgerät, maschine=Maschine, sonstiges=Sonstiges\n  "hersteller": string | null, // Hersteller\n  "modell": string | null, // Modell\n  "seriennummer": string | null, // Seriennummer\n  "kaufdatum": string | null, // YYYY-MM-DD\n  "standort": string | null, // Standort / Lagerort\n  "zustand": LookupValue | null, // Zustand (select one key: "neuwertig" | "gut" | "gebraucht" | "defekt" | "in_reparatur") mapping: neuwertig=Neuwertig, gut=Gut, gebraucht=Gebraucht, defekt=Defekt, in_reparatur=In Reparatur\n  "pruefpflichtig": boolean | null, // Prüfpflichtig (DGUV)\n  "naechste_pruefung": string | null, // YYYY-MM-DD\n  "bemerkung_werkzeug": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Werkzeugkatalog['fields']>;
      });
      ...
      if (file.type.startsWith('image/') || file.type === 'application/pdf') {
        try {
          ...
          setFields(prev => ({ ...prev, foto_werkzeug: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Werkzeugkatalog bearbeiten' : 'Werkzeugkatalog hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="pruefpflichtig">Prüfpflichtig (DGUV)</Label>
            <div className="flex items-center gap-2 pt-1">
              <Checkbox
                id="pruefpflichtig"
                checked={!!fields.pruefpflichtig}
                onCheckedChange={(v) => setFields(f => ({ ...f, pruefpflichtig: !!v }))}
              />
              <Label htmlFor="pruefpflichtig" className="font-normal">Prüfpflichtig (DGUV)</Label>
            </div>
          </div>
          <div className="space-y-2">
            <Label htmlFor="naechste_pruefung">Nächste Prüfung fällig</Label>
            <Input
          ...
          <div className="space-y-2">
            <Label htmlFor="foto_werkzeug">Foto des Werkzeugs</Label>
            {fields.foto_werkzeug ? (
              <div className="flex items-center gap-3 rounded-lg border p-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm truncate text-foreground">{fields.foto_werkzeug.split("/").pop()}</p>
                  <div className="flex gap-2 mt-1">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setFields(f => ({ ...f, foto_werkzeug: fileUrl }));
                    ...
                    <button
                      ...
                      onClick={() => setFields(f => ({ ...f, foto_werkzeug: undefined }))}
                    >
                      Entfernen
                    </button>
                  </div>
