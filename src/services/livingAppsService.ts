...
import type { Werkzeugausgabe, Werkzeugkatalog, WartungsUndPruefungsprotokoll, Mitarbeiterverwaltung, CreateWerkzeugausgabe, CreateWerkzeugkatalog, CreateWartungsUndPruefungsprotokoll, CreateMitarbeiterverwaltung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getWerkzeugausgabe(): Promise<Werkzeugausgabe[]> {
    ...
    return enrichLookupFields(records, 'werkzeugausgabe');
  }
  static async getWerkzeugausgabeEntry(id: string): Promise<Werkzeugausgabe | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Werkzeugausgabe;
    return enrichLookupFields([record], 'werkzeugausgabe')[0];
  }
  static async createWerkzeugausgabeEntry(fields: CreateWerkzeugausgabe) {
    return callApi('POST', `/apps/${APP_IDS.WERKZEUGAUSGABE}/records`, { fields: cleanFieldsForApi(fields as any, 'werkzeugausgabe') });
  }
  static async updateWerkzeugausgabeEntry(id: string, fields: Partial<CreateWerkzeugausgabe>) {
    return callApi('PATCH', `/apps/${APP_IDS.WERKZEUGAUSGABE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'werkzeugausgabe') });
  }
  ...
  // --- WERKZEUGKATALOG ---
  static async getWerkzeugkatalog(): Promise<Werkzeugkatalog[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGKATALOG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Werkzeugkatalog[];
    return enrichLookupFields(records, 'werkzeugkatalog');
  }
  static async getWerkzeugkatalogEntry(id: string): Promise<Werkzeugkatalog | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGKATALOG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Werkzeugkatalog;
    return enrichLookupFields([record], 'werkzeugkatalog')[0];
  }
  static async createWerkzeugkatalogEntry(fields: CreateWerkzeugkatalog) {
    return callApi('POST', `/apps/${APP_IDS.WERKZEUGKATALOG}/records`, { fields: cleanFieldsForApi(fields as any, 'werkzeugkatalog') });
  }
  static async updateWerkzeugkatalogEntry(id: string, fields: Partial<CreateWerkzeugkatalog>) {
    return callApi('PATCH', `/apps/${APP_IDS.WERKZEUGKATALOG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'werkzeugkatalog') });
  }
  static async deleteWerkzeugkatalogEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WERKZEUGKATALOG}/records/${id}`);
  }

  // --- WARTUNGS_UND_PRUEFUNGSPROTOKOLL ---
  static async getWartungsUndPruefungsprotokoll(): Promise<WartungsUndPruefungsprotokoll[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGS_UND_PRUEFUNGSPROTOKOLL}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as WartungsUndPruefungsprotokoll[];
    return enrichLookupFields(records, 'wartungs_und_pruefungsprotokoll');
  }
  static async getWartungsUndPruefungsprotokollEntry(id: string): Promise<WartungsUndPruefungsprotokoll | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGS_UND_PRUEFUNGSPROTOKOLL}/records/${id}`);
    const record = { record_id: data.id, ...data } as WartungsUndPruefungsprotokoll;
    return enrichLookupFields([record], 'wartungs_und_pruefungsprotokoll')[0];
  }
  static async createWartungsUndPruefungsprotokollEntry(fields: CreateWartungsUndPruefungsprotokoll) {
    return callApi('POST', `/apps/${APP_IDS.WARTUNGS_UND_PRUEFUNGSPROTOKOLL}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungs_und_pruefungsprotokoll') });
  }
  static async updateWartungsUndPruefungsprotokollEntry(id: string, fields: Partial<CreateWartungsUndPruefungsprotokoll>) {
    return callApi('PATCH', `/apps/${APP_IDS.WARTUNGS_UND_PRUEFUNGSPROTOKOLL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungs_und_pruefungsprotokoll') });
  }
  static async deleteWartungsUndPruefungsprotokollEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNGS_UND_PRUEFUNGSPROTOKOLL}/records/${id}`);
  }
