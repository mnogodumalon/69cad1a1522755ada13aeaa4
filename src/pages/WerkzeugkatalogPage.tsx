...
export default function WerkzeugkatalogPage() {
  const [records, setRecords] = useState<Werkzeugkatalog[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Werkzeugkatalog | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Werkzeugkatalog | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Werkzeugkatalog | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getWerkzeugkatalog());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Werkzeugkatalog['fields']) {
    await LivingAppsService.createWerkzeugkatalogEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Werkzeugkatalog['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateWerkzeugkatalogEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteWerkzeugkatalogEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Werkzeugkatalog im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Werkzeugkatalog suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('pruefpflichtig')}>
                <span className="inline-flex items-center gap-1">
                  Prüfpflichtig (DGUV)
                  {sortKey === 'pruefpflichtig' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('foto_werkzeug')}>
                <span className="inline-flex items-center gap-1">
                  Foto des Werkzeugs
                  {sortKey === 'foto_werkzeug' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.pruefpflichtig ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.pruefpflichtig ? 'Ja' : 'Nein'}</span></TableCell>
                ...
                <TableCell>{record.fields.foto_werkzeug ? <div className="relative h-8 w-8 rounded bg-muted overflow-hidden"><div className="absolute inset-0 flex items-center justify-center"><IconFileText size={14} className="text-muted-foreground" /></div><img src={record.fields.foto_werkzeug} alt="" className="relative h-full w-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} /></div> : '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={14} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Werkzeugkatalog. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
